...
try:
    #logging.debug('===>In Resolve-Script, Params: {}'.format(json.dumps(params, indent=4)))
    url = None
    if "mac" in params:
        mac = str(params["mac"])
        mac = ':'.join(mac[i:i+2] for i in range(0, 12, 2)).upper()
        logging.debug('===>Mac address ' + mac + ' found . Sending request')
        url = base_url+"/Rest/Devices?mac="+mac+"&include=location,subcategory,asset-info,connectivity-info,clinical-info"
    elif "ip" in params:
        ip = str(params["ip"])
        logging.debug('===>IpAddress ' + ip + ' found . Sending request')
        url = base_url+"/Rest/Devices?ip="+ip+"&include=location,subcategory,asset-info,connectivity-info,clinical-info"
    else:
        logging.error("*************Neither MacAddress or IpAddress resolved from Params**************")
        response["error"] = "Neither MAC nor IP found in params"
        skip_processing = True
    
    # Only call API if we have a valid URL
    if not skip_processing and url:
        #logging.debug("Ordr API to fetch the Device Info : {}".format(url))
        ...
        if resp.status_code == 200 :
            ...
            #logging.debug("Ordr API Response: {}".format(json.dumps(detail, indent=4)))
            if "error" in detail:
                logging.debug("Ordr reponse=> Info not available for request : {}".format(url))
                response["error"] = "Device info not available: {}".format(url)
                skip_processing = True
                
            if not skip_processing and len(detail) > 0 :
                ...
                if "isBlacklisted" in detail:
                    properties["connect_ordrhealthcare_blocklistStatus"] = str(detail['isBlacklisted'])
                    logging.debug("connect_ordrhealthcare_blocklistStatus : " + str(detail['isBlacklisted']))
                else:
                    properties["connect_ordrhealthcare_blocklistStatus"] = ""
                ...
                logging.debug("Ordr-to-Forescout mapping was a success")
                #logging.debug("=======>>>>>Ordr: response returned: {response}, URL: {url}".format(response=response, url=url))
            else:
                logging.debug("Record not found for request: {}".format(url))
                response["error"] = "Record not found for request: "+str(url)
        else:
            response["error"] = "Connection Error to Ordr : "+str(resp.status_code)
            logging.debug("Connection Error to Ordr : "+str(resp.status_code))
    else:
        response["error"] = "Ordr API to fetch the Device Info : " + url
        logging.error("Ordr API to fetch the Device Info : {}".format(url))
