"""
Proxmox Connect App - Cancel Resume (Suspend)
Undoes a Resume action by suspending the QEMU VM.
"""
...
try:
    if not vmid or not node:
        ...
        logging.info(f"Proxmox cancel-resume: suspending qemu/{vmid} on {node}")
    ...
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to cancel resume (suspend) qemu/{vmid}: {str(e)}"
