...
def _extract_indexed_keys(data, prefix):
    """Return keys that look like '<prefix><number>' sorted by numeric suffix."""
    indexed = []
...
def parse_macs_from_config(config, guest_type):
    """Extract all configured guest MAC addresses from net* config entries.

    Returns:
        list of unique MAC addresses as 12-char uppercase hex strings
    """
    if not config:
        return []
    ...
    for net_key in _extract_indexed_keys(config, "net"):
        mac = parse_mac_from_net_config(config.get(net_key, ""), guest_type)
...
def _normalize_mac(mac_raw):
    """Normalize MAC address to 12-char uppercase hex string."""
    if not mac_raw or ":" not in mac_raw or len(mac_raw) != 17:
        return None
    return mac_raw.replace(":", "").upper()


def parse_ips_from_interfaces(interfaces, guest_type):
    """Extract all non-loopback IPv4 addresses from guest interface data.

    Returns:
        list of unique IPv4 strings
    """
    if not interfaces:
        return []
    ...
    for iface in interfaces:
        ...
        if guest_type == "qemu":
            ...
            if isinstance(inet, list):
                for value in inet:
                    ip = value.split("/")[0] if isinstance(value, str) else ""
                    if ip and not ip.startswith("127."):
                        iface_ips.append(ip)
            elif isinstance(inet, str) and inet:
                ip = inet.split("/")[0]
                if ip and not ip.startswith("127."):
                    iface_ips.append(ip)
...
def parse_ips_from_config(config, guest_type):
    """Extract all configured static IPv4 addresses from guest config.

    Returns:
        list of unique IPv4 strings (dhcp entries are ignored)
    """
    if not config:
        return []
    ...
    if guest_type == "lxc":
        config_keys = _extract_indexed_keys(config, "net")
    else:
        config_keys = _extract_indexed_keys(config, "ipconfig")

    for key in config_keys:
        raw = config.get(key, "")
        if not raw:
            continue
        match = re.search(r'ip=(\d+\.\d+\.\d+\.\d+)', raw)
        if match:
            ip = match.group(1)
            if ip not in seen:
                seen.add(ip)
...
def parse_interface_identities(interfaces, guest_type):
    """Extract per-interface identities containing MAC and IPv4 addresses.

    Returns:
        list of dicts: {"mac": <normalized-mac-or-None>, "ips": [ipv4, ...]}
    """
    if not interfaces:
        return []
    ...
    for iface in interfaces:
        ...
        if guest_type == "qemu":
            mac = _normalize_mac(iface.get("hardware-address", ""))
            ips = []
            for addr in iface.get("ip-addresses", []):
                if addr.get("ip-address-type") == "ipv4":
                    ip = addr.get("ip-address", "")
            ...
            mac = _normalize_mac(iface.get("hwaddr", ""))
            ips = []
            inet = iface.get("inet", "")
            if isinstance(inet, list):
                for value in inet:
        ...
        if mac or unique_ips:
            identities.append({"mac": mac, "ips": unique_ips})
