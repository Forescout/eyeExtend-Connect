"""
Proxmox Connect App - Shutdown Action
Gracefully shuts down a VM or LXC container via ACPI signal.
The guest OS performs a clean shutdown. Requires QEMU guest agent for VMs.
"""
...
try:
    ...
    logging.error(f"Proxmox shutdown failed: HTTP {status_code}")
except Exception as e:
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to shutdown {guest_type}/{vmid}: {str(e)}"
