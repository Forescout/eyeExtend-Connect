"""
Proxmox Connect App - Resume Action
...
try:
    if not vmid or not node or not guest_type:
        ...
        response["succeeded"] = False
        response["troubleshooting"] = "Resume is only supported for QEMU VMs, not LXC containers."
    else:
        logging.info(f"Proxmox action: resuming qemu/{vmid} on {node}")
        ...
        logging.info(f"Proxmox action: resume qemu/{vmid} succeeded")
    ...
    logging.error(f"Proxmox resume failed: HTTP {status_code}")
except Exception as e:
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to resume qemu/{vmid}: {str(e)}"
