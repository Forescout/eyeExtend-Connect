...
try:
    ...
    for guest in guests:
        ...
        # Step 2: Get config for MAC addresses, ostype, description, and static IPs
        mac_addresses = []
        config = None
        try:
            ...
            # Extract all configured MAC addresses from net* entries
            mac_addresses = proxmox_lib.parse_macs_from_config(config, guest_type)
            logging.info(
                f"Proxmox poll: {guest_type}/{vmid} configured MACs={mac_addresses}"
            )
        ...
        if is_running:
            ...
            interface_identities = proxmox_lib.parse_interface_identities(
                interfaces, guest_type
            )
            ip_addresses = proxmox_lib.parse_ips_from_interfaces(interfaces, guest_type)

        # Add configured static IPs from config (works for stopped guests too)
        if config:
            config_ips = proxmox_lib.parse_ips_from_config(config, guest_type)
            for ip in config_ips:
                if ip not in ip_addresses:
                    ip_addresses.append(ip)
            if config_ips:
                logging.info(
                    f"Proxmox poll: {guest_type}/{vmid} configured IPs={config_ips}"
                )
        ...
        # Step 5: Build endpoints with identifiers for all discovered NICs/IPs
        endpoint_identities = []
        seen_identity = set()

        # Prefer live interface pairings when available.
        for identity in interface_identities:
            iface_mac = identity.get("mac")
            iface_ips = identity.get("ips", [])
            if iface_ips:
                for iface_ip in iface_ips:
                    key = (iface_mac or "", iface_ip or "")
                    if key not in seen_identity:
                        seen_identity.add(key)
                        endpoint_identities.append({"mac": iface_mac, "ip": iface_ip})
            elif iface_mac:
                key = (iface_mac, "")
                if key not in seen_identity:
                    seen_identity.add(key)
                    endpoint_identities.append({"mac": iface_mac, "ip": None})

        # Add configured MACs and IPs that were not visible in live interfaces.
        for mac in mac_addresses:
            key = (mac, "")
            if key not in seen_identity:
                seen_identity.add(key)
                endpoint_identities.append({"mac": mac, "ip": None})

        for ip in ip_addresses:
            key = ("", ip)
            if key not in seen_identity:
                seen_identity.add(key)
                endpoint_identities.append({"mac": None, "ip": ip})

        logging.info(
            f"Proxmox poll: {guest_type}/{vmid} ({name}) discovered "
            f"{len(endpoint_identities)} identities "
            f"(configured_macs={len(mac_addresses)} live_or_config_ips={len(ip_addresses)})"
        )
        ...
        for identity in endpoint_identities:
            ...
            if identity.get("mac"):
                endpoint["mac"] = identity["mac"]
            if identity.get("ip"):
                endpoint["ip"] = identity["ip"]
            endpoints.append(endpoint)
            logging.info(
                f"Proxmox poll: adding endpoint {guest_type}/{vmid} ({name}) "
                f"mac={identity.get('mac')} ip={identity.get('ip')} "
                f"online={is_running}"
            )
