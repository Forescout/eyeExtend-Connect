"""
Proxmox Connect App - Cancel Start (Stop)
Undoes a Start action by stopping the VM/container.
"""
...
try:
    if not vmid or not node or not guest_type:
        ...
        logging.info(f"Proxmox cancel-start: stopping {guest_type}/{vmid} on {node}")
    ...
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to cancel start (stop) {guest_type}/{vmid}: {str(e)}"
