...
#  Version: 2.2.0 (adds connect_wingetpatcher_outdated_pkg list property
#           for the Inventory view; version now tracks the app version)
# ============================================================
#
#  CHANGE FROM 0.4.2:
...
if not raw_output:
    ...
    # Step 4 (added v2.2.0): Inventory-facing list property.
    # Each line of parsed['apps'] is "PackageId (current -> available)".
    # The package ID is the first whitespace-delimited token on the line.
    # One list element per package ID -> the Inventory view renders one
    # row per package with a count of hosts reporting it outdated.
    #
    # DELIBERATE CHOICE: we OMIT the property when the list is empty
    # rather than returning [] - an empty list's behavior in the
    # framework response is unverified, and a rejected response would
    # also break the composite property above (the production update
    # flow). Cost: a fully-patched host keeps its last non-empty list
    # value until the property expires from cache, so Inventory host
    # counts can lag slightly behind reality. Field-test note: try
    # returning [] on a single test host; if the framework accepts and
    # clears the value, remove this guard.
    pkg_ids = []
    if parsed.get("apps"):
        for line in parsed["apps"].split("\n"):
            line = line.strip()
            if not line or line.startswith("ERROR_"):
                continue
            pkg_ids.append(line.split(" ", 1)[0].strip())
    if pkg_ids:
        properties["connect_wingetpatcher_outdated_pkg"] = pkg_ids
        logging.debug(
            "WinGetPatcher resolver: outdated_pkg list = {}".format(pkg_ids)
        )
