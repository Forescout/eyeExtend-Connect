"""
OpenShift Virt Connect App - Unpause VM Action
Unpauses a paused VirtualMachineInstance via the KubeVirt subresource API.
"""
...
try:
    if not vm_name or not namespace:
        ...
        logging.info(f"OpenShift Virt action: unpausing VM {namespace}/{vm_name}")
        openshiftvirt_lib.vm_subresource_action(
            base_url=base_url, namespace=namespace, name=vm_name,
            action="unpause", token=token,
            verify=ssl_verify, proxies=proxies
        )
        response["succeeded"] = True
        logging.info(f"OpenShift Virt action: unpause VM {namespace}/{vm_name} succeeded")
    ...
    response["troubleshooting"] = (
        ...
        f"Check ServiceAccount RBAC: needs 'update' on virtualmachineinstances/unpause subresource."
    )
    logging.error(f"OpenShift Virt unpause failed: HTTP {status_code}")
except Exception as e:
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to unpause VM {namespace}/{vm_name}: {str(e)}"
