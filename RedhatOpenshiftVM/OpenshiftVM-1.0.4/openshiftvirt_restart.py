"""
OpenShift Virt Connect App - Restart VM Action
Restarts a running VirtualMachine via the KubeVirt subresource API.
"""
...
try:
    if not vm_name or not namespace:
        ...
        logging.info(f"OpenShift Virt action: restarting VM {namespace}/{vm_name}")
        openshiftvirt_lib.vm_subresource_action(
            base_url=base_url, namespace=namespace, name=vm_name,
            action="restart", token=token,
            verify=ssl_verify, proxies=proxies
        )
        response["succeeded"] = True
        logging.info(f"OpenShift Virt action: restart VM {namespace}/{vm_name} succeeded")
    ...
    response["troubleshooting"] = (
        ...
        f"Check ServiceAccount RBAC: needs 'update' on virtualmachines/restart subresource."
    )
    logging.error(f"OpenShift Virt restart failed: HTTP {status_code}")
except Exception as e:
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to restart VM {namespace}/{vm_name}: {str(e)}"
