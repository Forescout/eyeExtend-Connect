"""
OpenShift Virt Connect App - Cancel Start (Stop)
Undoes a Start action by stopping the VM.
"""
...
try:
    if not vm_name or not namespace:
        ...
        logging.info(f"OpenShift Virt cancel-start: stopping VM {namespace}/{vm_name}")
    ...
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to cancel start (stop) VM {namespace}/{vm_name}: {str(e)}"
