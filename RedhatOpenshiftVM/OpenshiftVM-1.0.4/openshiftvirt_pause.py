"""
OpenShift Virt Connect App - Pause VM Action
Pauses a running VirtualMachineInstance via the KubeVirt subresource API.
"""
...
try:
    if not vm_name or not namespace:
        ...
        logging.info(f"OpenShift Virt action: pausing VM {namespace}/{vm_name}")
        ...
        logging.info(f"OpenShift Virt action: pause VM {namespace}/{vm_name} succeeded")
    ...
    response["troubleshooting"] = (
        ...
        f"Check ServiceAccount RBAC: needs 'update' on virtualmachineinstances/pause subresource."
    )
    logging.error(f"OpenShift Virt pause failed: HTTP {status_code}")
except Exception as e:
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to pause VM {namespace}/{vm_name}: {str(e)}"
