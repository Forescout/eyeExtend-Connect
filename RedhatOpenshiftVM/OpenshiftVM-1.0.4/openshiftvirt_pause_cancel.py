"""
OpenShift Virt Connect App - Cancel Pause (Unpause)
Undoes a Pause action by unpausing the VM.
"""
...
try:
    if not vm_name or not namespace:
        ...
        logging.info(f"OpenShift Virt cancel-pause: unpausing VM {namespace}/{vm_name}")
    ...
    logging.exception(e)
    response["succeeded"] = False
    response["troubleshooting"] = f"Failed to cancel pause (unpause) VM {namespace}/{vm_name}: {str(e)}"
