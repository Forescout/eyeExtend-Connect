...
original_event = str(params.get("connect_elastic_preserve_original_event")).lower() in ("true", "1")
duplicate_field = str(params.get("connect_elastic_preserve_duplicate_custom_field")).lower() in ("true", "1")
properties = {}
batch_mode = False
item_response = {}
...
def fetch_data() -> List[Dict]:
    ...
    try:
        for host in endpoints:
            ...
            prop_tags = host.get("property_tags", {})
            if isinstance(prop_tags, str):
                prop_tags = json.loads(prop_tags)
            elif not isinstance(prop_tags, dict):
                raise ValueError("property_tags must be a dict or JSON string")

            prop_tags["timestamp"] = datetime.now().isoformat()
            host["property_tags"] = prop_tags
...
def create_payload(forescout_data: List[Dict]) -> str:
    ...
    for data in forescout_data:
        ...
        tags = ["forescout-host"]
        if original_event:
            tags.append("preserve_original_event")
        if duplicate_field:
            tags.append("preserve_duplicate_custom_field")

        body = {"message": json.dumps(data.get("property_tags", {}))}
        if tags:
            body["tags"] = tags
