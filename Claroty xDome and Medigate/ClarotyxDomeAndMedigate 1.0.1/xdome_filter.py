...
BASE_QUERY_FILTERS = [
    BASE_UNIQUE_ID_CONDITION_FILTER | FIRST_SEEN_CONDITION_FILTER,
    NOT_RETIRED_DEVICE_CONDITION_FILTER,
]


def parse_text_filter_value(value: str) -> List[str]:
    list_of_values = []
...
def validate_cidr(cidr: str) -> bool:
    return True if re.match(cidr_regex_pattern, cidr) else False
...
def parse_subnet_filter(value: str) -> Optional[QueryFilter]:
    ...
    try:
        ...
        if not isinstance(list_of_values, list):
            list_of_values = [str(list_of_values)]
    except (SyntaxError, ValueError):
        pass
